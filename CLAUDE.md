...
- **Guardrail chống connection mồ côi nằm ở mức ROLE, KHÔNG ở connection option** (`drizzle/0004_db_guardrails.sql`) — đây là gốc rễ của hiện tượng "lúc được lúc không" (sự cố 31/08, tái phát 01/09 ngay sau khi lên v6).
  ...
  - **Lưu ý khi vận hành:** thiết lập mức role chỉ áp cho phiên MỚI. Sau khi đổi, connection cũ mà Supavisor đang giữ vẫn mang giá trị cũ — phải `pg_terminate_backend` chúng thì mới ăn ngay. Và vì `statement_timeout` giờ là 15s cho role `postgres`, migration nào chạy lâu hơn 15s phải tự mở đầu bằng `SET statement_timeout = 0;`.
